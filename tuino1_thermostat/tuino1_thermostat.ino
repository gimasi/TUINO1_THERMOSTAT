...
float thermostat_temperature = 21.0;
int manual_mode = 0;
int relay_status = 0;
...
void loop() {
  ...
  int port;
   ...
   if ( manual_mode == 0 ) {
    ...
    if ( thermostat_temperature != 0 ) {
      ...
      if (readTemp()<=thermostat_temperature) {
            relay_status = 1;  
      }
      else 
      {
  ...
  temp_buffer[temperature_counts] = readTemp();
  temp_temperature += temp_buffer[temperature_counts];
  temperature_counts += 1;
  
  if ( temperature_counts == 20 )
  {
    current_temperature = temp_temperature / 20;
